...
export function ShardDetails(props: {
  shard: ShardView;
  history: HistoryControls
}) {
  const rarityClass = props.shard.rarity;

  return <div class="shard-details">
    <div class="row" >
      <div class="column">
        <img class="shard-image" src="/shard_placeholder.png" />
      </div>
      <div class="column">
        <span class={["shard-name", rarityClass].join(" ")}>{props.shard.id} {props.shard.name}</span> ({props.shard.attributeName})
      </div>
      <div class="column">
        Buy price: {formatNumber(props.shard.bazaarPrice)}
      </div>
    </div>
    <div class="row">
      <div class="column">
        <span class="shard-effect-description">{formatEffect(props.shard.effectDescription, props.shard.effectMax, props.shard.effect2Max)}</span>
      </div>
    </div>
    <div class="row">
      <div class="column">
    ...
    <div class="row">
      <div class="column">
        Basic fuse target: {props.shard.isBasicFuseTarget ? "YES" : "NO"}
      </div>
    </div>
    <div class="row">
      <div class="column">
        Special fuses: {props.shard.specialFusesDesc ? props.shard.specialFusesDesc.map(pair => pair.join(" + ")).join(", ") : "NONE"}
      </div>
    </div>
    <div class="row">
      <div class="column">
        ...
        <table>
          ...
          <tbody>
            {props.shard.valuatedFuses.map(function(fuse) {
              ...
              return (
                <tr>
                  <td onClick={() => props.history.selectNewShard(fuse.shard1)}>{fuse.shard1} {fuse.shard1Name}</td>
                  <td onClick={() => props.history.selectNewShard(fuse.shard2)}>{fuse.shard2} {fuse.shard2Name}</td>

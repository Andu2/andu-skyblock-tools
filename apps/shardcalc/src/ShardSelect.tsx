...
export function ShardSelect(props: {
  vm: ShardViewModel;
  ...
  function getFilteredShards() {
    ...
    return props.vm.shardIds.filter(shardId => {
      ...
      return shard.id.toLowerCase().includes(term) ||
  ...
  return (
    <div>
      <div class="history-controls">
        <button disabled={!props.history.canGoBack()} onClick={props.history.goBack}>Back</button>
        <button disabled={!props.history.canGoForward()} onClick={props.history.goForward}>Forward</button>
      </div>
      <div class="selected-shard">
        Selected Shard: {props.selectedShard || "None"}
      </div>
      <input
        type="text"
